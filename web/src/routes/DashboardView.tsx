...
function stateOf(row: Artifact | undefined): string {
	...
	if (state === "measured" || state === "inferred" || state === "unknown")
		return state;
	return "unknown";
}
...
function NumberTile({
	...
	const stale =
		row &&
		(tile.stale_after_seconds ?? 0) > 0 &&
		age > (tile.stale_after_seconds ?? 0);
	return (
		<div
		...
		>
			...
			{row ? (
				<>
					<div
						data-tile-value
						className="py-1 font-semibold text-2xl tabular-nums text-right"
					>
						{readingOf(row)}
					</div>
					<div
...
function GridTile({
	...
	return (
		...
		>
			...
			{row && !grid ? (
				<div className="py-1 text-muted-foreground text-sm">
					its newest reading is not a grid of {"{cols, rows}"} - the tile says
					so rather than drawing a wrong-shaped matrix
				</div>
			) : !row || !grid ? (
				<div className="py-1 text-muted-foreground text-sm">
					its metric has no rows pushed yet - not zero, just nothing
...
function TableTile({
	...
	rows,
	now,
}: { tile: DashboardTile; rows: Artifact[]; now: number }) {
	return (
		<div
			data-tile-label={tile.label}
				...
				<ul className="mt-1 flex flex-col">
					{rows.map((row) => (
						...
						>
							...
							<span className="ml-auto font-medium tabular-nums text-right">
								{readingOf(row)}
							</span>
						</li>
					))}
	...
	if (!signedIn) {
		return (
			<div className="px-4 py-6 text-muted-foreground text-sm">
				log in, or paste a token, to see this dashboard - signed out this is a
				locked shelf, not an empty one
			</div>
		);
	}
	if (refused) {
		return (
			<div
				data-dashboard-refused
				className="px-4 py-6 text-muted-foreground text-sm"
			>
				this dashboard is not readable by you - it lives outside the projects
				you can read
			</div>
		);
	}
		...
		<div className="flex h-full flex-col">
			<header className="flex flex-wrap items-center gap-2 border-border border-b px-4 py-3">
				...
				<span className="ml-auto text-muted-foreground text-xs">
					{dash.project ?? "no project"} - drawn from pushed rows, nothing runs
					here
				</span>
			</header>
			...
			>
				{tiles.map((tile) => {
					...
					return (
						...
						>
							{tile.label}: tile kind {tile.kind} is not in this console's
							vocabulary
						</div>
					);
				})}
