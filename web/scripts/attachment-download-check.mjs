 ...
 *
 * WRITTEN BY lubuntu3-glm on branch lubuntu3-glm/an-attachment-row-shows-its-file
 * and ported here by claude-host, because 3e907dd landed the feature without a
 * guard and this is the guard. 01M3BHV07Z871GTVWZQZPSQ42Z. Three things moved:
 * the save hook is data-attachment-save as they named it (master had called it
 * data-attachment-download), the card is asked for eagerly by the page rather
 * than by this check, and flow 3 no longer clicks - see there.
 *
...
like the file.`);
  ...
  // NOTHING IS CLICKED HERE, which is this port's one deviation from the check as
  // it was written. It clicked the control and expected it to explain itself; the
  // reason now REPLACES the control, so there is nothing to press. A control that
  // explains itself only when pressed is still a control that appears to do
  // nothing, and the assertion is the same either way: the case is named on
  // screen, and no file comes away.
  ...
  if ((await why.count()) === 0) {
    die(`the node answered with content:null and the card said nothing about it.
It has to name the case - the row is readable and its payload is not here - or a reader
looking for a way to keep the file finds neither the file nor a sentence about why.`);
  }
  if ((await page.locator(`[data-attachment-save="${item.id}"]`).count()) !== 0) {
    die(`the node answered with content:null and a save control was offered anyway.
The href would be a data: URL over the word "undefined", which hands the reader a file
that is not the attachment and calls it one.`);
  }
  // Belt: the reason being present must not come with a download happening from
