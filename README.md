# flowy - Handoff Fabric node (Phase 1)
...
| route | what it does |
| --- | --- |
| `POST /api/artifacts` | create, or replace one you own. Body: `type` (required), `title`, `body`, `discovery`, `status`, `severity`, `tags`, `user_tags`, `related`, `visibility`, `project`, `file_path`, `fields`, `id?`. A new `id` is a ULID; `hlc` and `node` are stamped |
| `GET /api/artifacts?type=&project=&status=` | `{"artifacts":[...]}`, permission-filtered, newest first, tombstones omitted |
...
| `GET /api/search?q=&type=&project=` | `{"query":..., "artifacts":[{..., "rank":...}]}`, ranked and permission-filtered |
...
Green. `./run-tests.sh` reports `passed: 58 failed: 0` on Ubuntu 24.04 with Go
1.22 and Postgres 16 - the 21 Phase 0 checks and 37 more.
