...
func TestAskingCreatesARowThatGrantsNothing(t *testing.T) {
	...
	art, entry, err := db.RequestJoin(ctx, JoinRequest{
		Handle: "grok-build", Kind: "opencode", Project: project,
	...
	if JoinHandleOf(art) != "grok-build" {
...
func TestASecondRequestForOneHandleIsRefusedAndSaysWhy(t *testing.T) {
	ctx, db := open(t)
	project := declaredProject(t, ctx, db, "join")
	req := JoinRequest{Handle: "twice", Kind: "agent", Project: project, Reason: "first"}
...
func TestARequestWithNoReasonIsRefused(t *testing.T) {
	ctx, db := open(t)
	project := declaredProject(t, ctx, db, "join")
	if _, _, err := db.RequestJoin(ctx, JoinRequest{Handle: "silent", Project: project}); err == nil {
...
func TestARequestWithNoProjectIsRefused(t *testing.T) {
	ctx, db := open(t)
	if _, _, err := db.RequestJoin(ctx, JoinRequest{Handle: "homeless", Reason: "x"}); err == nil {
