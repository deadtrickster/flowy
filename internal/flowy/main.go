...
// buildStamp names the build itself. cmd/flowy holds the -ldflags symbol
// of the same name and hands it in through Run, so the build sets it:
//
...
// Run is the CLI entry: it parses args, dispatches to the verb, and returns
// the process exit code. cmd/flowy hands it the build stamp it was built with;
// everything else - the usage text, the version math, the verb dispatch - lives
// here in one place, so a test can drive the whole CLI without a subprocess.
func Run(args []string, stamp string) int {
	if stamp != "" {
		buildStamp = stamp
		version = versionOf(release, buildStamp)
	}
	if len(args) < 1 {
	...
	case "inbox":
		...
		if err := inboxCmd(args[1:]); err != nil {
	...
	case "worklog":
		...
		if err := worklogCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy worklog: %v\n", err)
			return 2
		}
	case "tui":
		if err := tuiCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy tui: %v\n", err)
			return 1
		}
	case "fuse":
		if err := fuseCmd(args[1:]); err != nil {
	...
	case "queue":
		...
		if err := queueCmd(args[1:]); err != nil {
	...
	case "waiter":
		...
		if err := waiterCmd(args[1:]); err != nil {
	...
	case "nag":
		if err := nagCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy nag: %v\n", err)
			return 2
		}
	case "projects":
		if err := projectsCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy projects: %v\n", err)
			return 1
		}
	case "sync":
		if err := syncCmd(args[1:]); err != nil {
	...
	case "identity":
		if err := identityCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy identity: %v\n", err)
			return 1
		}
	case "mint":
		if err := mintCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy mint: %v\n", err)
			return 1
		}
	case "passwd":
		if err := passwdCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy passwd: %v\n", err)
			return 1
		}
	case "todo":
		if err := todoCmd(args[1:]); err != nil {
	...
	case "merge":
		if err := mergeCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy merge: %v\n", err)
			return 1
		}
	case "principal":
		if err := principalCmd(args[1:]); err != nil {
			fmt.Fprintf(os.Stderr, "flowy principal: %v\n", err)
			return 1
		}
	case "sign":
		if err := signCmd(args[1:]); err != nil {
